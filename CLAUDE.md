...
- Cross-reference chips on rec cards — every rec persists `crossReferences: { title, reason }[]` produced by the score step (zod-constrained, schema enforced). UI shows "Because you loved [Title]" chips; clicking opens an evidence modal that surfaces the matching profile theme/archetype quotes via `titleAppearsIn` (now lifted to `packages/shared/src/titleMatch.ts` so client + server share one matcher).
- Profile evolution timeline — `GET /api/profile/versions` returns the user's full version history. Client computes structural diffs between adjacent versions (themes added/removed/weighted, archetypes, avoidances, dislikedTitles, format comfort) — no AI cost, deterministic, defensible. Rendered on `/profile` collapsed by default ("Show all N versions →").
