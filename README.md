...
![Signed-out landing: editorial hero with magazine-style typography](./screenshots/landing.jpg)
...
- **Shared media-type glyph vocabulary.** Movies render as a circle, TV as a triangle, anime as a hexagon, games as a diamond, manga as a 5-point star, books as a 4-point sparkle. The same shape vocabulary as Constellation's node glyphs — the two apps speak one visual language for media types.
- **Watchlist + plan-to.** Mark a rec as "Plan to" or import your plan-to lists from any source. Watchlist items are deduped from future recommendations but don't anchor cross-references (the user hasn't actually engaged with them yet). Each watchlist row has inline 5-star rating and a *Mark watched* control — marking watched promotes the item to the consumed library and triggers AI annotation server-side, so a rated rewatch immediately joins the profile signal. A *Pick for me* button next to the mood-decider picks one item uniformly at random when none of the mood prompts feel right.
- **Watchlist enrichment.** Watchlist rows render with posters, year, runtime, and a 1-sentence blurb, pulled from the same `media_cache` the recommender uses (one source of truth for media metadata). New adds enrich inline on save; pre-existing watchlist items auto-backfill the first time you visit `/watchlist` via a small batch endpoint that defaults to status=watchlist so consumed-library rows don't eat the budget. Per-format filter tabs above the list (Movies / TV / Anime / Manga / Games / Books) hide zero-count formats; un-enrichable rows fall back gracefully to text.
- **Sort + filter.** Recommendations sortable by match %, alphabetical, or year (asc/desc), per format tab, URL-synced.
- **Doomscroll view toggle.** A `Grid` / `One at a time` toggle on the recommendations page switches between the dense 2-col card grid and a single-card-per-row focus layout. localStorage-persisted, grid stays the default.
